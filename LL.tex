...
\begin{align*}
  \layout{A⊗B} = \layout{A \pa B}  &= \layout A · \layout B\\
   \layout{A ⊕ B} = \layout{A \& B} &= \text{tag (1 bit) + 1 pointer (optionally null if the data is not ready)} \\
...
\item ⊕: wait for the data to be ready; then chose a branch according
  to the tag. Free the pointer and the tag.
...
\item Cut/Quantifiers: the $∀$-side environment contains a pointer to
  a closure capable of copying data of the new type.
...
\item Ax: Copy the data between the closures. For type variables, use
  the closure to that avail from the environment.
...
TODO: Should we do reference-counting?


\subsection{Abstract machine (variant 2)}
