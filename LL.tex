...
\begin{mathpar}
\inferrule[Ax]{ }{Θ; x : \p A, y : A ⊢ x ↔ y}
\and
\inferrule[Cut]{Θ; Γ, x:\p A ⊢ a \\ y:A, Δ ⊢ b }{Θ; Γ,Δ ⊢ \ConnectS {\newchan A} x a y b}
\\
\inferrule[⊗]{Θ; Γ, x : A, y : B, Δ ⊢ a}{Θ; Γ, z : A ⊗ B, Δ ⊢ \Let ⟨x,y⟩=z \In a}
\and 
\inferrule[\pa]{Θ; Γ, x:A ⊢ a\\ Θ; y:B, Δ ⊢ b}{Θ; Γ, z:A\pa B, Δ ⊢ \ConnectS z x a y b}
\\
\begin{array}{c}
\inferrule[\&-l]{Θ; Γ,x:A ⊢ a} {Θ; Γ,z:A~\&~B ⊢ \Let x = \Fst z \In a}\\
\\ 
\inferrule[\&-r]{Θ; Γ,y:B ⊢ b} {Θ; Γ,z:A~\&~B ⊢ \Let y = \Snd z \In b}\\
\end{array}
\and
\inferrule[⊕]{Θ;Γ,x:A ⊢ a \\ Γ,y:B ⊢ b} {Θ; Γ,z:A⊕B ⊢ \Case z \Of \{ \Inl x ↦ a ; \Inr y ↦ b  \} }
\\
\inferrule[1]{Θ;Γ ⊢ a}{Θ;Γ, x:1 ⊢ \Let ⋄ = x \In a} 
\and
\inferrule[⊥]{ }{Θ;x:⊥ ⊢ x} 
\\
\text{no rule for ⊤}
\and
\inferrule[0]{ }{Θ;Γ,x:0 ⊢ \mathsf{dump~}Γ\mathsf{~into~} x} 
\\
  \inferrule[∃]{Θ,α;Γ,y:A ⊢ b}{Θ;Γ,z:∃α.A ⊢ \Let ⟨α,y⟩ = z \In b }
\and
  \inferrule[∀]{Θ;Γ,x:A[B/α] ⊢ b \\ Θ ⊢ B}{Θ;Γ,z:∀α.A ⊢ \Let x = z∙B \In b }
\\
  \inferrule[?]{Θ;!Γ,x:A ⊢ a}{Θ;!Γ,x:?A ⊢ \mathsf{offer~}x \In a }
\and
  \inferrule[!]{Θ;Γ,x:A ⊢ a}{Θ;Γ,x:!A ⊢ \mathsf{demand~}x \In a }
\\
  \inferrule[Weaken]{Θ;Γ ⊢ a}{Θ;Γ,x:!A ⊢ \mathsf{ignore~}x \In a }
\and
  \inferrule[Contract]{Θ;Γ,x:!A,y:!A ⊢ a}{Θ;Γ,x:!A ⊢ \Let y = \mathsf{alias~}x \In a }
\end{mathpar}
...
\begin{mathpar}
\ConnectB{!A} x {\Let y = \mathsf{alias~}x \In a} z {\mathsf{offer~}z \In b} ~⟶~ \ConnectB{!A} x {\ConnectB{!A} y {a} z {\mathsf{offer~}z \In b}} z {\mathsf{offer~}z \In b}
%\ConnectB{!A} x {a} z {\mathsf{offer~}z \In b}
%\ConnectB{!A} y {a} z {\mathsf{offer~}z \In b}
\end{mathpar}

Alternate rule for Contract

\begin{mathpar}
\ConnectB{!A} x {\Let y = \mathsf{alias~}x \In a} z {\mathsf{offer~}z \In b} ~⟶~ \ConnectB{!A} x {\mathsf{demand~}x \In a[x/y]} z {\mathsf{offer~}z \In b}
\end{mathpar}
